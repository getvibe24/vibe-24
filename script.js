  ...
  if (zoneUsers.length === 0) {
    feedContainer.innerHTML = `
      <div class="glass-card" style="text-align: center; padding: 40px 20px;">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width: 32px; height: 32px; color: var(--text-muted); margin-bottom: 8px;"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/></svg>
        <h4 style="margin-top: 4px;">Quiet Zone</h4>
        <p style="font-size: 12.5px; color: var(--text-secondary); margin-top: 4px;">
          No active vibes found in ${appState.currentArea} right now. Try switching broad areas.
        </p>
      </div>
    `;
    return;
      ...
      <div class="user-card-header">
        <div class="anon-badge">
          <div class="anon-avatar">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="8" r="4"/><path d="M6 21v-2a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v2"/></svg>
          </div>
...
function simulateIncomingNotification(senderUser) {
  ...
  container.innerHTML = `
    <h4 style="font-size: 15px; margin-bottom: 4px; color: var(--text-primary);">Anonymous Signal Received</h4>
    <p style="font-size: 12.5px; color: var(--text-secondary);">Someone in <strong>${senderUser.area}</strong> is interested in your vibe.</p>
    <div class="tags-container" style="justify-content: center; margin-top: 14px;">
...
function triggerMutualMatch(matchedUser) {
  ...
  card.innerHTML = `
    <div style="text-align: center; margin-bottom: 14px;">
      <h3 style="font-size: 16px; margin-top: 4px; color: var(--text-primary);">Connected in ${matchedUser.area}</h3>
    </div>
    <p style="font-size: 12.5px; color: var(--text-secondary); line-height: 1.5; margin-bottom: 14px; text-align: center;">
      You both expressed interest in connecting within the same broad area without sharing personal identity.
    </p>
    <div style="background: rgba(0,0,0,0.2); padding: 12px; border-radius: 12px; border: 1px solid var(--border-subtle);">
      <span style="font-size: 10px; font-weight: 700; color: var(--accent-primary); letter-spacing: 0.05em;">SHARED VIBE TAGS</span>
...
function initSafetyPage() {
  document.getElementById('simulateBlockBtn').addEventListener('click', () => {
    showToast('User blocked and reported anonymously.');
  });
}
